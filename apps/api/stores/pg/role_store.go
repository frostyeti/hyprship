...
func (s *RoleStore) Get(ctx context.Context, id uuid.UUID) (*models.Role, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("pg", query), id).Scan(&r.ID, &r.Name, &r.Description)
...
func (s *RoleStore) Create(ctx context.Context, role *models.Role) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", query), role.ID, role.Name, role.NameUpcase, role.Description)
	return err
}
...
func (s *RoleStore) Update(ctx context.Context, role *models.Role) error {
	...
	role.NameUpcase = strings.ToUpper(role.Name)
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", query), role.Name, role.NameUpcase, role.Description, role.ID)
	return err
}

func (s *RoleStore) Delete(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", `DELETE FROM roles WHERE id = ?`), id)
	return err
}
...
func (s *RoleStore) ListClaims(ctx context.Context, roleID uuid.UUID) ([]models.RoleClaim, error) {
	...
	rows, err := s.db.QueryContext(ctx, core.Rebind("pg", query), roleID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *RoleStore) AddClaim(ctx context.Context, claim *models.RoleClaim) error {
	query := `INSERT INTO role_claims (role_id, type, claim_value) VALUES (?, ?, ?) RETURNING id`
	err := s.db.QueryRowContext(ctx, core.Rebind("pg", query), claim.RoleID, claim.Type, claim.Value).Scan(&claim.ID)
	return err
}
