...
func (s *UserStore) List(ctx context.Context, opts core.ListOptions) (core.ListResult[models.User], error) {
	...
	// Simplistic sorting
	if opts.Sort != "" {
		query += " ORDER BY " + opts.Sort // Unsafe, assume validated by core API
		if opts.SortDesc {
			query += " DESC"
		}
	} else {
	...
	if opts.Filter != "" {
		...
		countArgs = args[:2] // take only the filter args
	}
...
func (s *UserStore) Import(items []models.User) error {
	...
	for _, user := range items {
		...
		_, err := stmt.Exec(user.ID, user.PrimaryEmail, user.PrimaryEmailUpcase, user.PrimaryPhone, user.Name, user.NameUpcase, user.ImageURI, user.IsBanned, createdAt, updatedAt)
		if err != nil {
			return err
		}
	}
