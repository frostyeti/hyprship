...
func (s *UserStore) List(ctx context.Context, opts core.ListOptions) (core.ListResult[models.User], error) {
	query := `SELECT CAST(id AS CHAR(36)), primary_email, primary_phone, name, image_uri, is_banned, created_at, updated_at FROM users`
	args := []any{}
	if opts.Filter != "" {
...
func (s *UserStore) Import(items []models.User) error {
	...
	for _, user := range items {
		...
		_, err := stmt.Exec(user.ID.String(), user.PrimaryEmail, user.PrimaryEmailUpcase, user.PrimaryPhone, user.Name, user.NameUpcase, user.ImageURI, user.IsBanned, user.CreatedAt, user.UpdatedAt)
		if err != nil {
			return err
		}
	}
