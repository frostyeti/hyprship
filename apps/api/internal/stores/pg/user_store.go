...
func (s *UserStore) Get(ctx context.Context, id uuid.UUID) (*models.User, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("pg", query), id).Scan(&u.ID, &u.PrimaryEmail, &u.PrimaryPhone, &u.Name, &u.ImageURI, &u.IsBanned, &createdAt, &updatedAt)
...
func (s *UserStore) GetByEmail(ctx context.Context, email string) (*models.User, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("pg", query), emailUpcase).Scan(&u.ID, &u.PrimaryEmail, &u.PrimaryPhone, &u.Name, &u.ImageURI, &u.IsBanned, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *UserStore) Create(ctx context.Context, user *models.User) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", query), user.ID, user.PrimaryEmail, user.PrimaryEmailUpcase, user.PrimaryPhone, user.Name, user.NameUpcase, user.ImageURI, user.IsBanned, user.CreatedAt, user.UpdatedAt)
	return err
}
...
func (s *UserStore) Update(ctx context.Context, user *models.User) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", query), user.PrimaryEmail, user.PrimaryEmailUpcase, user.PrimaryPhone, user.Name, user.NameUpcase, user.ImageURI, user.IsBanned, user.UpdatedAt, user.ID)
	return err
}

func (s *UserStore) Delete(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", `DELETE FROM users WHERE id = ?`), id)
	return err
}
...
func (s *UserStore) ListClaims(ctx context.Context, userID uuid.UUID) ([]models.UserClaim, error) {
	...
	rows, err := s.db.QueryContext(ctx, core.Rebind("pg", query), userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *UserStore) AddClaim(ctx context.Context, claim *models.UserClaim) error {
	query := `INSERT INTO user_claims (user_id, type, value) VALUES (?, ?, ?) RETURNING id`
	err := s.db.QueryRowContext(ctx, core.Rebind("pg", query), claim.UserID, claim.Type, claim.Value).Scan(&claim.ID)
	return err
}

func (s *UserStore) RemoveClaim(ctx context.Context, claimID int32) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", `DELETE FROM user_claims WHERE id = ?`), claimID)
	return err
}
