...
func (s *userTotpStore) GetByUserID(ctx context.Context, userID uuid.UUID) (*models.UserTotp, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("pg", query), userID.String()).Scan(
...
func (s *userTotpStore) Upsert(ctx context.Context, totp *models.UserTotp) error {
	...
	totp.UpdatedAt = &now
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", query), totp.ID.String(), totp.UserID.String(), totp.Secret, totp.IsVerified, totp.CreatedAt, totp.UpdatedAt)
	return err
}

func (s *userTotpStore) Delete(ctx context.Context, userID uuid.UUID) error {
	query := `DELETE FROM user_totps WHERE user_id = ?`
	_, err := s.db.ExecContext(ctx, core.Rebind("pg", query), userID.String())
	return err
}
