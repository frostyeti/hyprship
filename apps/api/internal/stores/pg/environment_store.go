...
func (s *EnvironmentStore) Get(ctx context.Context, id uuid.UUID) (*models.Environment, error) {
	...
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), id).Scan(&idStr, &pIdStr, &e.Name, &e.Description, &createdAt, &updatedAt)
...
func (s *EnvironmentStore) GetByName(ctx context.Context, projectID uuid.UUID, name string) (*models.Environment, error) {
	...
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &e.Name, &e.Description, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *EnvironmentStore) Create(ctx context.Context, env *models.Environment) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", query), env.ID, env.ProjectID, env.Name, env.NameUpcase, env.Description, env.CreatedAt, env.UpdatedAt)
...
func (s *EnvironmentStore) Update(ctx context.Context, env *models.Environment) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", query), env.Name, env.NameUpcase, env.Description, updatedAt, env.ID)

	return err
}

func (s *EnvironmentStore) Delete(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", "DELETE FROM environments WHERE id = ?"), id)
	return err
}
