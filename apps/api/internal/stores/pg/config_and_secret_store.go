...
func (s *ConfigStore) ListConfigFileNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.ConfigFileName], error) {
	...
	query, listArgs := core.BuildListQuery("postgres", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)

	rows, err := s.db.QueryContext(ctx, core.Rebind("postgres", query), finalArgs...)
	if err != nil {
	...
	err = s.db.QueryRowContext(ctx, core.Rebind("postgres", countQuery), finalCountArgs...).Scan(&total)
...
func (s *ConfigStore) GetConfigFileName(ctx context.Context, id uuid.UUID) (*models.ConfigFileName, error) {
	...
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *ConfigStore) GetConfigFileNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.ConfigFileName, error) {
	...
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ConfigStore) CreateConfigFileName(ctx context.Context, name *models.ConfigFileName) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", query), name.ID, name.ProjectID, name.Name, name.NameUpcase, name.CreatedAt, name.UpdatedAt)
	return err
}

func (s *ConfigStore) DeleteConfigFileName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", "DELETE FROM config_file_names WHERE id = ?"), id)
	return err
}
...
func (s *ConfigStore) GetConfigFile(ctx context.Context, nameID, envID uuid.UUID) (*models.ConfigFile, error) {
	...
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &f.Content, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ConfigStore) UpsertConfigFile(ctx context.Context, file *models.ConfigFile) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), file.ID, file.ConfigFileNameID, file.EnvironmentID, file.Content, file.CreatedAt, file.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE config_files SET content = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), file.Content, now, existing.ID)
	return err
}
...
func (s *ConfigStore) DeleteEnvVariableName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", "DELETE FROM env_variable_names WHERE id = ?"), id)
	return err
}
...
func (s *ConfigStore) GetEnvVariable(ctx context.Context, nameID, envID uuid.UUID) (*models.EnvVariable, error) {
	...
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &v.Value, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ConfigStore) UpsertEnvVariable(ctx context.Context, v *models.EnvVariable) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), v.ID, v.EnvVariableNameID, v.EnvironmentID, v.Value, v.CreatedAt, v.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE env_variables SET value = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), v.Value, now, existing.ID)
	return err
}
...
func (s *SecretStore) DeleteSecretName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", "DELETE FROM secret_names WHERE id = ?"), id)
	return err
}
...
func (s *SecretStore) GetSecret(ctx context.Context, nameID, envID uuid.UUID) (*models.Secret, error) {
	...
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &sec.Value, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *SecretStore) UpsertSecret(ctx context.Context, sec *models.Secret) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), sec.ID, sec.SecretNameID, sec.EnvironmentID, sec.Value, sec.CreatedAt, sec.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE secrets SET value = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), sec.Value, now, existing.ID)
	return err
}
...
func (s *SecretStore) DeleteCertificateName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", "DELETE FROM certificate_names WHERE id = ?"), id)
	return err
}
...
func (s *SecretStore) GetCertificate(ctx context.Context, nameID, envID uuid.UUID) (*models.Certificate, error) {
	...
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &cert.CertificateData, &privateKeySecret, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *SecretStore) UpsertCertificate(ctx context.Context, cert *models.Certificate) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), cert.ID, cert.CertificateNameID, cert.EnvironmentID, cert.CertificateData, cert.PrivateKeySecret, cert.CreatedAt, cert.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE certificates SET certificate_data = ?, private_key_secret = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), cert.CertificateData, cert.PrivateKeySecret, now, existing.ID)
	return err
}
...
func (s *SecretStore) DeleteSSHKeyName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("postgres", "DELETE FROM ssh_key_names WHERE id = ?"), id)
	return err
}
...
func (s *SecretStore) GetSSHKey(ctx context.Context, nameID, envID uuid.UUID) (*models.SSHKey, error) {
	...
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("postgres", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &key.PublicKey, &key.PrivateKeySecret, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *SecretStore) UpsertSSHKey(ctx context.Context, key *models.SSHKey) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), key.ID, key.SSHKeyNameID, key.EnvironmentID, key.PublicKey, key.PrivateKeySecret, key.CreatedAt, key.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE ssh_keys SET public_key = ?, private_key_secret = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("postgres", query), key.PublicKey, key.PrivateKeySecret, now, existing.ID)
	return err
}
