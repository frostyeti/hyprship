...
func (s *GroupStore) List(ctx context.Context, opts core.ListOptions) (core.ListResult[models.Group], error) {
	...
	baseQuery := `SELECT CAST(id AS CHAR(36)) AS id, name, description, image_uri, email, is_active, created_at, updated_at FROM [groups]`
...
func (s *GroupStore) Get(ctx context.Context, id uuid.UUID) (*models.Group, error) {
	var g models.Group
	var createdAt, updatedAt sql.NullTime
	query := `SELECT CAST(id AS CHAR(36)), name, description, image_uri, email, is_active, created_at, updated_at FROM [groups] WHERE id = ?`
	err := s.db.QueryRowContext(ctx, core.Rebind("mssql", query), id.String()).Scan(&g.ID, &g.Name, &g.Description, &g.ImageURI, &g.Email, &g.IsActive, &createdAt, &updatedAt)
...
func (s *GroupStore) GetByName(ctx context.Context, name string) (*models.Group, error) {
	...
	var g models.Group
	var createdAt, updatedAt sql.NullTime
	query := `SELECT CAST(id AS CHAR(36)), name, description, image_uri, email, is_active, created_at, updated_at FROM [groups] WHERE name_upcase = ?`
	err := s.db.QueryRowContext(ctx, core.Rebind("mssql", query), nameUpcase).Scan(&g.ID, &g.Name, &g.Description, &g.ImageURI, &g.Email, &g.IsActive, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *GroupStore) Create(ctx context.Context, group *models.Group) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", query), group.ID.String(), group.Name, group.NameUpcase, group.Description, group.ImageURI, group.Email, group.EmailUpcase, group.IsActive, group.CreatedAt, group.UpdatedAt)
	return err
}
...
func (s *GroupStore) Update(ctx context.Context, group *models.Group) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", query), group.Name, group.NameUpcase, group.Description, group.ImageURI, group.Email, group.EmailUpcase, group.IsActive, updatedAt, group.ID.String())
	return err
}

func (s *GroupStore) Delete(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", `DELETE FROM [groups] WHERE id = ?`), id.String())
	return err
}
...
func (s *GroupStore) ListUsers(ctx context.Context, groupID uuid.UUID) ([]models.GroupUser, error) {
	query := `SELECT CAST(group_id AS CHAR(36)), CAST(user_id AS CHAR(36)) FROM groups_users WHERE group_id = ?`
	rows, err := s.db.QueryContext(ctx, core.Rebind("mssql", query), groupID.String())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *GroupStore) ListAdmins(ctx context.Context, groupID uuid.UUID) ([]models.GroupAdmin, error) {
	query := `SELECT CAST(group_id AS CHAR(36)), CAST(user_id AS CHAR(36)) FROM groups_admins WHERE group_id = ?`
	rows, err := s.db.QueryContext(ctx, core.Rebind("mssql", query), groupID.String())
	if err != nil {
		return nil, err
	}
...
func (s *GroupStore) AddRole(ctx context.Context, groupID, roleID uuid.UUID) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", query), groupID.String(), roleID.String())
	return err
}
...
func (s *GroupStore) ListRoles(ctx context.Context, groupID uuid.UUID) ([]models.GroupRole, error) {
	query := `SELECT CAST(group_id AS CHAR(36)), CAST(role_id AS CHAR(36)) FROM groups_roles WHERE group_id = ?`
	rows, err := s.db.QueryContext(ctx, core.Rebind("mssql", query), groupID.String())
	if err != nil {
		return nil, err
	}
