...
func (s *ProjectStore) List(ctx context.Context, opts core.ListOptions) (core.ListResult[models.Project], error) {
	...
	baseQuery := `SELECT CAST(id AS CHAR(36)) AS id, name, slug, description, is_active, created_at, updated_at FROM projects`
...
func (s *ProjectStore) Get(ctx context.Context, id uuid.UUID) (*models.Project, error) {
	var p models.Project
	var createdAt, updatedAt sql.NullTime
	query := `SELECT CAST(id AS CHAR(36)), name, slug, description, is_active, created_at, updated_at FROM projects WHERE id = ?`
	err := s.db.QueryRowContext(ctx, core.Rebind("mssql", query), id.String()).Scan(&p.ID, &p.Name, &p.Slug, &p.Description, &p.IsActive, &createdAt, &updatedAt)
...
func (s *ProjectStore) GetBySlug(ctx context.Context, slug string) (*models.Project, error) {
	var p models.Project
	var createdAt, updatedAt sql.NullTime
	query := `SELECT CAST(id AS CHAR(36)), name, slug, description, is_active, created_at, updated_at FROM projects WHERE slug = ?`
	err := s.db.QueryRowContext(ctx, core.Rebind("mssql", query), slug).Scan(&p.ID, &p.Name, &p.Slug, &p.Description, &p.IsActive, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ProjectStore) Create(ctx context.Context, project *models.Project) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", query), project.ID.String(), project.Name, project.NameUpcase, project.Slug, project.Description, project.IsActive, project.CreatedAt, project.UpdatedAt)
	return err
}
...
func (s *ProjectStore) Update(ctx context.Context, project *models.Project) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", query), project.Name, project.NameUpcase, project.Slug, project.Description, project.IsActive, updatedAt, project.ID.String())
	return err
}

func (s *ProjectStore) Delete(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", `DELETE FROM projects WHERE id = ?`), id.String())
	return err
}
...
func (s *ProjectStore) AddGroup(ctx context.Context, projectID, groupID uuid.UUID, permissions int64) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", query), projectID.String(), groupID.String(), permissions)
	return err
}
...
func (s *ProjectStore) RemoveGroup(ctx context.Context, projectID, groupID uuid.UUID) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", query), projectID.String(), groupID.String())
	return err
}
...
func (s *ProjectStore) UpdateGroupPermissions(ctx context.Context, projectID, groupID uuid.UUID, permissions int64) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mssql", query), permissions, projectID.String(), groupID.String())
	return err
}

func (s *ProjectStore) ListGroups(ctx context.Context, projectID uuid.UUID) ([]models.ProjectGroup, error) {
	query := `SELECT CAST(project_id AS CHAR(36)), CAST(group_id AS CHAR(36)), permissions FROM projects_groups WHERE project_id = ?`
	rows, err := s.db.QueryContext(ctx, core.Rebind("mssql", query), projectID.String())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
