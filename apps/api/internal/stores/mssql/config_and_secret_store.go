...
func (s *ConfigStore) ListConfigFileNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.ConfigFileName], error) {
	...
	baseQuery := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM config_file_names WHERE project_id = ?"
...
func (s *ConfigStore) GetConfigFileName(ctx context.Context, id uuid.UUID) (*models.ConfigFileName, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM config_file_names WHERE id = ?"
	var n models.ConfigFileName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *ConfigStore) GetConfigFileNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.ConfigFileName, error) {
	...
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM config_file_names WHERE project_id = ? AND name_upcase = ?"
	var n models.ConfigFileName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ConfigStore) CreateConfigFileName(ctx context.Context, name *models.ConfigFileName) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlserver", query), name.ID, name.ProjectID, name.Name, name.NameUpcase, name.CreatedAt, name.UpdatedAt)
	return err
}

func (s *ConfigStore) DeleteConfigFileName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlserver", "DELETE FROM config_file_names WHERE id = ?"), id)
	return err
}

func (s *ConfigStore) GetConfigFile(ctx context.Context, nameID, envID uuid.UUID) (*models.ConfigFile, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(config_file_name_id AS VARCHAR(36)), CAST(environment_id AS VARCHAR(36)), content, created_at, updated_at FROM config_files WHERE config_file_name_id = ? AND environment_id = ?"
	var f models.ConfigFile
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &f.Content, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ConfigStore) UpsertConfigFile(ctx context.Context, file *models.ConfigFile) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), file.ID, file.ConfigFileNameID, file.EnvironmentID, file.Content, file.CreatedAt, file.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE config_files SET content = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), file.Content, now, existing.ID)
	return err
}
...
func (s *ConfigStore) ListEnvVariableNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.EnvVariableName], error) {
	...
	baseQuery := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM env_variable_names WHERE project_id = ?"
	query, listArgs := core.BuildListQuery("sqlserver", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)
...
func (s *ConfigStore) GetEnvVariableName(ctx context.Context, id uuid.UUID) (*models.EnvVariableName, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM env_variable_names WHERE id = ?"
	var n models.EnvVariableName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *ConfigStore) GetEnvVariableNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.EnvVariableName, error) {
	nameUpcase := strings.ToUpper(name)
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM env_variable_names WHERE project_id = ? AND name_upcase = ?"
	var n models.EnvVariableName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *ConfigStore) DeleteEnvVariableName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlserver", "DELETE FROM env_variable_names WHERE id = ?"), id)
	return err
}

func (s *ConfigStore) GetEnvVariable(ctx context.Context, nameID, envID uuid.UUID) (*models.EnvVariable, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(env_variable_name_id AS VARCHAR(36)), CAST(environment_id AS VARCHAR(36)), value, created_at, updated_at FROM env_variables WHERE env_variable_name_id = ? AND environment_id = ?"
	var v models.EnvVariable
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &v.Value, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ConfigStore) UpsertEnvVariable(ctx context.Context, v *models.EnvVariable) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), v.ID, v.EnvVariableNameID, v.EnvironmentID, v.Value, v.CreatedAt, v.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE env_variables SET value = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), v.Value, now, existing.ID)
	return err
}
...
func (s *SecretStore) ListSecretNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.SecretName], error) {
	...
	baseQuery := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM secret_names WHERE project_id = ?"
	query, listArgs := core.BuildListQuery("sqlserver", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)
...
func (s *SecretStore) GetSecretName(ctx context.Context, id uuid.UUID) (*models.SecretName, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM secret_names WHERE id = ?"
	var n models.SecretName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) GetSecretNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.SecretName, error) {
	nameUpcase := strings.ToUpper(name)
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM secret_names WHERE project_id = ? AND name_upcase = ?"
	var n models.SecretName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) DeleteSecretName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlserver", "DELETE FROM secret_names WHERE id = ?"), id)
	return err
}

func (s *SecretStore) GetSecret(ctx context.Context, nameID, envID uuid.UUID) (*models.Secret, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(secret_name_id AS VARCHAR(36)), CAST(environment_id AS VARCHAR(36)), value, created_at, updated_at FROM secrets WHERE secret_name_id = ? AND environment_id = ?"
	var sec models.Secret
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &sec.Value, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *SecretStore) UpsertSecret(ctx context.Context, sec *models.Secret) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), sec.ID, sec.SecretNameID, sec.EnvironmentID, sec.Value, sec.CreatedAt, sec.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE secrets SET value = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), sec.Value, now, existing.ID)
	return err
}
...
func (s *SecretStore) ListCertificateNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.CertificateName], error) {
	...
	baseQuery := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM certificate_names WHERE project_id = ?"
	query, listArgs := core.BuildListQuery("sqlserver", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)
...
func (s *SecretStore) GetCertificateName(ctx context.Context, id uuid.UUID) (*models.CertificateName, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM certificate_names WHERE id = ?"
	var n models.CertificateName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) GetCertificateNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.CertificateName, error) {
	nameUpcase := strings.ToUpper(name)
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM certificate_names WHERE project_id = ? AND name_upcase = ?"
	var n models.CertificateName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) DeleteCertificateName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlserver", "DELETE FROM certificate_names WHERE id = ?"), id)
	return err
}

func (s *SecretStore) GetCertificate(ctx context.Context, nameID, envID uuid.UUID) (*models.Certificate, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(certificate_name_id AS VARCHAR(36)), CAST(environment_id AS VARCHAR(36)), certificate_data, private_key_secret, created_at, updated_at FROM certificates WHERE certificate_name_id = ? AND environment_id = ?"
	var cert models.Certificate
	var idStr, nIdStr, eIdStr string
	var privateKeySecret sql.NullString
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &cert.CertificateData, &privateKeySecret, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *SecretStore) UpsertCertificate(ctx context.Context, cert *models.Certificate) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), cert.ID, cert.CertificateNameID, cert.EnvironmentID, cert.CertificateData, cert.PrivateKeySecret, cert.CreatedAt, cert.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE certificates SET certificate_data = ?, private_key_secret = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), cert.CertificateData, cert.PrivateKeySecret, now, existing.ID)
	return err
}
...
func (s *SecretStore) ListSSHKeyNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.SSHKeyName], error) {
	...
	baseQuery := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM ssh_key_names WHERE project_id = ?"
	query, listArgs := core.BuildListQuery("sqlserver", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)
...
func (s *SecretStore) GetSSHKeyName(ctx context.Context, id uuid.UUID) (*models.SSHKeyName, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM ssh_key_names WHERE id = ?"
	var n models.SSHKeyName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) GetSSHKeyNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.SSHKeyName, error) {
	nameUpcase := strings.ToUpper(name)
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(project_id AS VARCHAR(36)), name, created_at, updated_at FROM ssh_key_names WHERE project_id = ? AND name_upcase = ?"
	var n models.SSHKeyName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) DeleteSSHKeyName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlserver", "DELETE FROM ssh_key_names WHERE id = ?"), id)
	return err
}

func (s *SecretStore) GetSSHKey(ctx context.Context, nameID, envID uuid.UUID) (*models.SSHKey, error) {
	query := "SELECT CAST(id AS VARCHAR(36)), CAST(ssh_key_name_id AS VARCHAR(36)), CAST(environment_id AS VARCHAR(36)), public_key, private_key_secret, created_at, updated_at FROM ssh_keys WHERE ssh_key_name_id = ? AND environment_id = ?"
	var key models.SSHKey
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlserver", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &key.PublicKey, &key.PrivateKeySecret, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *SecretStore) UpsertSSHKey(ctx context.Context, key *models.SSHKey) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), key.ID, key.SSHKeyNameID, key.EnvironmentID, key.PublicKey, key.PrivateKeySecret, key.CreatedAt, key.UpdatedAt)
		return err
	}

	now := time.Now()
	query := "UPDATE ssh_keys SET public_key = ?, private_key_secret = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlserver", query), key.PublicKey, key.PrivateKeySecret, now, existing.ID)
	return err
}
