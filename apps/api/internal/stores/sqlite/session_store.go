...
func (s *UserSessionStore) ListByUser(ctx context.Context, userID uuid.UUID) ([]models.UserSession, error) {
	query := `SELECT id, user_id, expires_at, token, ip_address, user_agent, created_at, updated_at FROM user_sessions WHERE user_id = ? ORDER BY created_at DESC`
...
func (s *UserSessionStore) Create(ctx context.Context, session *models.UserSession) error {
	...
	_, err := s.db.ExecContext(ctx, query, session.ID, session.UserID, session.ExpiresAt.Unix(), session.Token, session.IPAddress, session.UserAgent, session.CreatedAt.Unix(), updatedAt)
	return err
}
...
func (s *UserSessionStore) Update(ctx context.Context, session *models.UserSession) error {
	...
	now := time.Now().Unix()
	_, err := s.db.ExecContext(ctx, query, session.ExpiresAt.Unix(), session.Token, session.IPAddress, session.UserAgent, now, session.ID)
	return err
}
