...
func (s *ConfigStore) ListConfigFileNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.ConfigFileName], error) {
	...
	baseQuery := "SELECT id, project_id, name, created_at, updated_at FROM config_file_names WHERE project_id = ?"
...
func (s *ConfigStore) GetConfigFileName(ctx context.Context, id uuid.UUID) (*models.ConfigFileName, error) {
	query := "SELECT id, project_id, name, created_at, updated_at FROM config_file_names WHERE id = ?"
	var n models.ConfigFileName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *ConfigStore) GetConfigFileNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.ConfigFileName, error) {
	...
	query := "SELECT id, project_id, name, created_at, updated_at FROM config_file_names WHERE project_id = ? AND name_upcase = ?"
	var n models.ConfigFileName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ConfigStore) CreateConfigFileName(ctx context.Context, name *models.ConfigFileName) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlite", query), name.ID, name.ProjectID, name.Name, name.NameUpcase, func() int64 { return name.CreatedAt.Unix() }(), func() *int64 {
		if name.UpdatedAt != nil {
			t := name.UpdatedAt.Unix()
			return &t
		}
		return nil
	}())
...
func (s *ConfigStore) DeleteConfigFileName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlite", "DELETE FROM config_file_names WHERE id = ?"), id)
	return err
}

func (s *ConfigStore) GetConfigFile(ctx context.Context, nameID, envID uuid.UUID) (*models.ConfigFile, error) {
	query := "SELECT id, config_file_name_id, environment_id, content, created_at, updated_at FROM config_files WHERE config_file_name_id = ? AND environment_id = ?"
	var f models.ConfigFile
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &f.Content, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
	...
	if createdAt.Valid {
		f.CreatedAt = time.Unix(createdAt.Int64, 0)
	}
	if updatedAt.Valid {
		t := time.Unix(updatedAt.Int64, 0)
		f.UpdatedAt = &t
...
func (s *ConfigStore) UpsertConfigFile(ctx context.Context, file *models.ConfigFile) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), file.ID, file.ConfigFileNameID, file.EnvironmentID, file.Content, func() int64 { return file.CreatedAt.Unix() }(), func() *int64 {
			if file.UpdatedAt != nil {
				t := file.UpdatedAt.Unix()
				return &t
			}
			return nil
		}())
	...
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), file.Content, time.Now().Unix(), existing.ID)
	return err
}
...
func (s *ConfigStore) ListEnvVariableNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.EnvVariableName], error) {
	...
	baseQuery := "SELECT id, project_id, name, created_at, updated_at FROM env_variable_names WHERE project_id = ?"
	query, listArgs := core.BuildListQuery("sqlite", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)
...
func (s *ConfigStore) GetEnvVariableName(ctx context.Context, id uuid.UUID) (*models.EnvVariableName, error) {
	query := "SELECT id, project_id, name, created_at, updated_at FROM env_variable_names WHERE id = ?"
	var n models.EnvVariableName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *ConfigStore) GetEnvVariableNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.EnvVariableName, error) {
	nameUpcase := strings.ToUpper(name)
	query := "SELECT id, project_id, name, created_at, updated_at FROM env_variable_names WHERE project_id = ? AND name_upcase = ?"
	var n models.EnvVariableName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *ConfigStore) DeleteEnvVariableName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlite", "DELETE FROM env_variable_names WHERE id = ?"), id)
	return err
}

func (s *ConfigStore) GetEnvVariable(ctx context.Context, nameID, envID uuid.UUID) (*models.EnvVariable, error) {
	query := "SELECT id, env_variable_name_id, environment_id, value, created_at, updated_at FROM env_variables WHERE env_variable_name_id = ? AND environment_id = ?"
	var v models.EnvVariable
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &v.Value, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ConfigStore) UpsertEnvVariable(ctx context.Context, v *models.EnvVariable) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), v.ID, v.EnvVariableNameID, v.EnvironmentID, v.Value, func() int64 { return v.CreatedAt.Unix() }(), func() *int64 {
			if v.UpdatedAt != nil {
				t := v.UpdatedAt.Unix()
				return &t
			}
	...
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), v.Value, time.Now().Unix(), existing.ID)
	return err
}
...
func (s *SecretStore) ListSecretNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.SecretName], error) {
	...
	baseQuery := "SELECT id, project_id, name, created_at, updated_at FROM secret_names WHERE project_id = ?"
	query, listArgs := core.BuildListQuery("sqlite", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)
...
func (s *SecretStore) GetSecretName(ctx context.Context, id uuid.UUID) (*models.SecretName, error) {
	query := "SELECT id, project_id, name, created_at, updated_at FROM secret_names WHERE id = ?"
	var n models.SecretName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) GetSecretNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.SecretName, error) {
	nameUpcase := strings.ToUpper(name)
	query := "SELECT id, project_id, name, created_at, updated_at FROM secret_names WHERE project_id = ? AND name_upcase = ?"
	var n models.SecretName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) DeleteSecretName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlite", "DELETE FROM secret_names WHERE id = ?"), id)
	return err
}

func (s *SecretStore) GetSecret(ctx context.Context, nameID, envID uuid.UUID) (*models.Secret, error) {
	query := "SELECT id, secret_name_id, environment_id, value, created_at, updated_at FROM secrets WHERE secret_name_id = ? AND environment_id = ?"
	var sec models.Secret
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &sec.Value, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
	...
	if createdAt.Valid {
		sec.CreatedAt = time.Unix(createdAt.Int64, 0)
	}
	if updatedAt.Valid {
		t := time.Unix(updatedAt.Int64, 0)
		sec.UpdatedAt = &t
...
func (s *SecretStore) UpsertSecret(ctx context.Context, sec *models.Secret) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), sec.ID, sec.SecretNameID, sec.EnvironmentID, sec.Value, func() int64 { return sec.CreatedAt.Unix() }(), func() *int64 {
			if sec.UpdatedAt != nil {
				t := sec.UpdatedAt.Unix()
				return &t
			}
			return nil
		}())
	...
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), sec.Value, time.Now().Unix(), existing.ID)
	return err
}
...
func (s *SecretStore) ListCertificateNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.CertificateName], error) {
	...
	baseQuery := "SELECT id, project_id, name, created_at, updated_at FROM certificate_names WHERE project_id = ?"
	query, listArgs := core.BuildListQuery("sqlite", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)
...
func (s *SecretStore) GetCertificateName(ctx context.Context, id uuid.UUID) (*models.CertificateName, error) {
	query := "SELECT id, project_id, name, created_at, updated_at FROM certificate_names WHERE id = ?"
	var n models.CertificateName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) GetCertificateNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.CertificateName, error) {
	nameUpcase := strings.ToUpper(name)
	query := "SELECT id, project_id, name, created_at, updated_at FROM certificate_names WHERE project_id = ? AND name_upcase = ?"
	var n models.CertificateName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) DeleteCertificateName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlite", "DELETE FROM certificate_names WHERE id = ?"), id)
	return err
}

func (s *SecretStore) GetCertificate(ctx context.Context, nameID, envID uuid.UUID) (*models.Certificate, error) {
	query := "SELECT id, certificate_name_id, environment_id, certificate_data, private_key_secret, created_at, updated_at FROM certificates WHERE certificate_name_id = ? AND environment_id = ?"
	var cert models.Certificate
	var idStr, nIdStr, eIdStr string
	var privateKeySecret sql.NullString
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &cert.CertificateData, &privateKeySecret, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
	...
	if createdAt.Valid {
		cert.CreatedAt = time.Unix(createdAt.Int64, 0)
	}
	if updatedAt.Valid {
		t := time.Unix(updatedAt.Int64, 0)
		cert.UpdatedAt = &t
...
func (s *SecretStore) UpsertCertificate(ctx context.Context, cert *models.Certificate) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), cert.ID, cert.CertificateNameID, cert.EnvironmentID, cert.CertificateData, cert.PrivateKeySecret, func() int64 { return cert.CreatedAt.Unix() }(), func() *int64 {
			if cert.UpdatedAt != nil {
				t := cert.UpdatedAt.Unix()
				return &t
			}
			return nil
		}())
	...
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), cert.CertificateData, cert.PrivateKeySecret, time.Now().Unix(), existing.ID)
	return err
}
...
func (s *SecretStore) ListSSHKeyNames(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.SSHKeyName], error) {
	...
	baseQuery := "SELECT id, project_id, name, created_at, updated_at FROM ssh_key_names WHERE project_id = ?"
	query, listArgs := core.BuildListQuery("sqlite", baseQuery, opts, fieldMap)
	finalArgs := append([]interface{}{projectID}, listArgs...)
...
func (s *SecretStore) GetSSHKeyName(ctx context.Context, id uuid.UUID) (*models.SSHKeyName, error) {
	query := "SELECT id, project_id, name, created_at, updated_at FROM ssh_key_names WHERE id = ?"
	var n models.SSHKeyName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), id).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) GetSSHKeyNameByName(ctx context.Context, projectID uuid.UUID, name string) (*models.SSHKeyName, error) {
	nameUpcase := strings.ToUpper(name)
	query := "SELECT id, project_id, name, created_at, updated_at FROM ssh_key_names WHERE project_id = ? AND name_upcase = ?"
	var n models.SSHKeyName
	var idStr, pIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), projectID, nameUpcase).Scan(&idStr, &pIdStr, &n.Name, &createdAt, &updatedAt)
...
func (s *SecretStore) DeleteSSHKeyName(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("sqlite", "DELETE FROM ssh_key_names WHERE id = ?"), id)
	return err
}

func (s *SecretStore) GetSSHKey(ctx context.Context, nameID, envID uuid.UUID) (*models.SSHKey, error) {
	query := "SELECT id, ssh_key_name_id, environment_id, public_key, private_key_secret, created_at, updated_at FROM ssh_keys WHERE ssh_key_name_id = ? AND environment_id = ?"
	var key models.SSHKey
	var idStr, nIdStr, eIdStr string
	var createdAt, updatedAt sql.NullInt64
	err := s.db.QueryRowContext(ctx, core.Rebind("sqlite", query), nameID, envID).Scan(&idStr, &nIdStr, &eIdStr, &key.PublicKey, &key.PrivateKeySecret, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *SecretStore) UpsertSSHKey(ctx context.Context, key *models.SSHKey) error {
	...
	if existing == nil {
		...
		_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), key.ID, key.SSHKeyNameID, key.EnvironmentID, key.PublicKey, key.PrivateKeySecret, func() int64 { return key.CreatedAt.Unix() }(), func() *int64 {
			if key.UpdatedAt != nil {
				t := key.UpdatedAt.Unix()
				return &t
			}
			return nil
		}())
	...
	_, err = s.db.ExecContext(ctx, core.Rebind("sqlite", query), key.PublicKey, key.PrivateKeySecret, time.Now().Unix(), existing.ID)
	return err
}
