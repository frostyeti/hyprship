...
func (s *GroupStore) Get(ctx context.Context, id uuid.UUID) (*models.Group, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), id).Scan(&g.ID, &g.Name, &g.Description, &g.ImageURI, &g.Email, &g.IsActive, &createdAt, &updatedAt)
...
func (s *GroupStore) GetByName(ctx context.Context, name string) (*models.Group, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), nameUpcase).Scan(&g.ID, &g.Name, &g.Description, &g.ImageURI, &g.Email, &g.IsActive, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *GroupStore) Create(ctx context.Context, group *models.Group) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), group.ID, group.Name, group.NameUpcase, group.Description, group.ImageURI, group.Email, group.EmailUpcase, group.IsActive, group.CreatedAt, group.UpdatedAt)
	return err
}
...
func (s *GroupStore) Update(ctx context.Context, group *models.Group) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), group.Name, group.NameUpcase, group.Description, group.ImageURI, group.Email, group.EmailUpcase, group.IsActive, updatedAt, group.ID)
	return err
}

func (s *GroupStore) Delete(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", `DELETE FROM `+"`groups`"+` WHERE id = ?`), id)
	return err
}
...
func (s *GroupStore) AddUser(ctx context.Context, groupID, userID uuid.UUID) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), groupID, userID)
	return err
}
...
func (s *GroupStore) ListUsers(ctx context.Context, groupID uuid.UUID) ([]models.GroupUser, error) {
	...
	rows, err := s.db.QueryContext(ctx, core.Rebind("mysql", query), groupID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *GroupStore) ListGroupsByUserID(ctx context.Context, userID uuid.UUID) ([]models.GroupUser, error) {
	...
	rows, err := s.db.QueryContext(ctx, core.Rebind("mysql", query), userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *GroupStore) AddRole(ctx context.Context, groupID, roleID uuid.UUID) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), groupID, roleID)
	return err
}
