...
func (s *ProjectStore) Get(ctx context.Context, id uuid.UUID) (*models.Project, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), id).Scan(&p.ID, &p.Name, &p.Slug, &p.Description, &p.IsActive, &createdAt, &updatedAt)
...
func (s *ProjectStore) GetBySlug(ctx context.Context, slug string) (*models.Project, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), slug).Scan(&p.ID, &p.Name, &p.Slug, &p.Description, &p.IsActive, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *ProjectStore) Create(ctx context.Context, project *models.Project) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), project.ID, project.Name, project.NameUpcase, project.Slug, project.Description, project.IsActive, project.CreatedAt, project.UpdatedAt)
	return err
}
...
func (s *ProjectStore) Update(ctx context.Context, project *models.Project) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), project.Name, project.NameUpcase, project.Slug, project.Description, project.IsActive, updatedAt, project.ID)
	return err
}

func (s *ProjectStore) Delete(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", `DELETE FROM projects WHERE id = ?`), id)
	return err
}
...
func (s *ProjectStore) AddGroup(ctx context.Context, projectID, groupID uuid.UUID, permissions int64) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), projectID, groupID, permissions)
	return err
}
...
func (s *ProjectStore) RemoveGroup(ctx context.Context, projectID, groupID uuid.UUID) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), projectID, groupID)
	return err
}
...
func (s *ProjectStore) UpdateGroupPermissions(ctx context.Context, projectID, groupID uuid.UUID, permissions int64) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), permissions, projectID, groupID)
	return err
}
...
func (s *ProjectStore) ListGroups(ctx context.Context, projectID uuid.UUID) ([]models.ProjectGroup, error) {
	...
	rows, err := s.db.QueryContext(ctx, core.Rebind("mysql", query), projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
