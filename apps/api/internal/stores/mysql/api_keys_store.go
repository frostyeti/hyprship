...
func (s *UserAPIKeyStore) Get(ctx context.Context, id int32) (*models.UserAPIKey, error) {
	query := `
		SELECT id, CAST(user_id AS CHAR(36)), name, name_upcase, key_hint, key_digest, expires_at, is_locked, is_revoked, comment, created_at, updated_at
		FROM user_api_keys
		WHERE id = ?
	`
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), id).Scan(
...
func (s *UserAPIKeyStore) Create(ctx context.Context, apiKey *models.UserAPIKey) error {
	...
	query := `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
	res, err := s.db.ExecContext(ctx, core.Rebind("mysql", query),
...
func (s *UserAPIKeyStore) Delete(ctx context.Context, id int32) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), id)
	return err
}
