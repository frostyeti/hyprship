...
func (s *EnvironmentStore) List(ctx context.Context, projectID uuid.UUID, opts core.ListOptions) (core.ListResult[models.Environment], error) {
	...
	baseQuery := "SELECT CAST(id AS CHAR(36)), CAST(project_id AS CHAR(36)), name, description, created_at, updated_at FROM environments WHERE project_id = ?"
...
func (s *EnvironmentStore) Get(ctx context.Context, id uuid.UUID) (*models.Environment, error) {
	query := "SELECT CAST(id AS CHAR(36)), CAST(project_id AS CHAR(36)), name, description, created_at, updated_at FROM environments WHERE id = ?"
	var e models.Environment
	var idStr, pIdStr string

	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), id.String()).Scan(&idStr, &pIdStr, &e.Name, &e.Description, &createdAt, &updatedAt)
...
func (s *EnvironmentStore) GetByName(ctx context.Context, projectID uuid.UUID, name string) (*models.Environment, error) {
	...
	query := "SELECT CAST(id AS CHAR(36)), CAST(project_id AS CHAR(36)), name, description, created_at, updated_at FROM environments WHERE project_id = ? AND name_upcase = ?"
	var e models.Environment
	var idStr, pIdStr string

	var createdAt, updatedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), projectID.String(), nameUpcase).Scan(&idStr, &pIdStr, &e.Name, &e.Description, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		return nil, err
...
func (s *EnvironmentStore) Create(ctx context.Context, env *models.Environment) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), env.ID.String(), env.ProjectID.String(), env.Name, env.NameUpcase, env.Description, env.CreatedAt, env.UpdatedAt)
...
func (s *EnvironmentStore) Update(ctx context.Context, env *models.Environment) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), env.Name, env.NameUpcase, env.Description, updatedAt, env.ID.String())

	return err
}

func (s *EnvironmentStore) Delete(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", "DELETE FROM environments WHERE id = ?"), id.String())
	return err
}
