...
func (s *userPasskeyStore) Get(ctx context.Context, id uuid.UUID) (*models.UserPasskey, error) {
	query := `
		SELECT CAST(id AS CHAR(36)), CAST(user_id AS CHAR(36)), credential_id, data, created_at
		FROM user_passkeys
		WHERE id = ?
	`
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), id.String()).Scan(
...
func (s *userPasskeyStore) GetByCredentialID(ctx context.Context, credentialID []byte) (*models.UserPasskey, error) {
	...
	err := s.db.QueryRowContext(ctx, core.Rebind("mysql", query), credentialID).Scan(
		&idStr, &userIDStr, &p.CredentialID, &p.Data, &p.CreatedAt,
	)
	if err != nil {
		return nil, err
...
func (s *userPasskeyStore) ListByUser(ctx context.Context, userID uuid.UUID) ([]models.UserPasskey, error) {
	...
	rows, err := s.db.QueryContext(ctx, core.Rebind("mysql", query), userID.String())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *userPasskeyStore) Create(ctx context.Context, passkey *models.UserPasskey) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query),
...
func (s *userPasskeyStore) Delete(ctx context.Context, id uuid.UUID) error {
	...
	_, err := s.db.ExecContext(ctx, core.Rebind("mysql", query), id.String())
	return err
}
